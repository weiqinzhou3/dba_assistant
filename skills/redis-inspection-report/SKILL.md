...
2. If log analysis is requested, call
   `redis_inspection_log_candidates(dataset_handle=...)`.
3. Call `review_redis_log_candidates(log_candidates_json=...)`. This tool must
   directly consume the candidate payload returned by
   `redis_inspection_log_candidates`; it must not use generic filesystem tools
   such as `ls`, `glob`, `grep`, or `read_file` to search for JSON files.
   Reviewed issues are still generated by LLM semantic review inside the tool.
